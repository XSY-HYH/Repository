...
function renderFileList(data) {
    ...
    if (data.files && data.files.length > 0) {
        data.files.forEach(file => {
            html += '<tr><td class=""file"">' + escapeHtml(file.name) + '</td><td>' + formatFileSize(file.size) + '</td><td class=""actions""><button class=""action-btn btn-delete"" onclick=""deleteFile(\'' + escapeHtml(file.path) + '\')"">回收站</button><button class=""action-btn btn-move"" onclick=""showMoveModal(\'' + escapeHtml(file.path) + '\', \'file\')"">移动</button></td></tr>';
        });
    }
